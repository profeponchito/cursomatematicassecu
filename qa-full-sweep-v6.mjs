...
async function probarPDA(page, grado, pda, consoleErrors) {
  ...
  for (let si = 0; si < pda.subtemas.length; si++) {
    ...
    // Comparación insensible a mayúsculas/minúsculas: overline_() (Paso 20)
    // envuelve esta etiqueta en un <p class="... uppercase ...">, y
    // innerText() de Playwright refleja el texto YA transformado por CSS
    // (todo en mayúsculas visualmente), no el texto literal del DOM.
    const bodyTextMin = bodyText.toLowerCase();
    if (subtema.nivelEtiqueta) {
      if (!bodyTextMin.includes(subtema.nivelEtiqueta.toLowerCase())) {
      ...
      if (!bodyTextMin.includes(`nivel ${si + 1} de ${totalNucleo}`) || !bodyTextMin.includes(nivelesEsperados[si].toLowerCase())) {
      ...
      if (!bodyTextMin.includes(`repaso ${indiceRepaso + 1} de ${totalRepasos}`)) {
    ...
    for (let parte = 0; parte < totalPartes; parte++) {
      ...
      // "Ronda N de M" también se renderiza dentro de overline_() (uppercase
      // CSS) — comparación insensible a mayúsculas, misma razón que arriba.
      if (totalPartes > 1 && !bodyText.toLowerCase().includes(`ronda ${parte + 1} de ${totalPartes}`)) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (candidatos.length > 1) {
          ...
          if (etiquetas.length > 0) {
            ...
            // Nota (Paso 20): cada celda es un <span>/<input> "inline-flex" —
            // con Tailwind realmente cargado, Chromium inserta un salto de
            // línea en innerText() alrededor de CUALQUIER caja flex/inline-flex
            // (comportamiento documentado del algoritmo de innerText, no un
            // bug de esta app), así que una fila con 2+ celdas visibles puede
            // devolver "3\n8" en vez de "38". Como firmaAlgoritmoColumnas_
            // nunca deja espacios DENTRO de una fila (solo entre filas, vía
            // el join('\n') de abajo), se quita TODO espacio/salto interno de
            // cada fila antes de comparar — el salto de línea ENTRE filas ya
            // viene de allInnerTexts() devolver un elemento por fila.
  ...
  bodyText = await page.innerText('body');
  // 'Resultado global del PDA' se renderiza dentro de overline_() (Paso 20:
  // <p class="... uppercase ...">) — comparación insensible a mayúsculas,
  // ver la misma nota más arriba sobre innerText() y CSS uppercase.
  if (!bodyText.toLowerCase().includes('resultado global del pda')) {
    ...
    for (const archivo of archivos) {
      ...
      try {
        ...
        if (resultados.length === 0) {
          // Espera activa (no un timeout fijo): con Tailwind/fuentes reales
          // cargando (o, en este sandbox, fallando tras su propio timeout de
          // red antes de que el navegador siga), el primer pintado de
          // /#/grados puede tardar un poco más que en pruebas anteriores.
          await page.waitForFunction(() => document.body.innerText.includes('Ejercítate'), { timeout: 5000 }).catch(() => {});
        ...
        if (!caminoVerificado.has(grado)) {
          ...
          // Paso 20: cada nodo es ahora <details class="mn-nodo"> — el enlace
          // real vive en la tarjeta flotante, oculta hasta presionar/enfocar el
          // círculo (<summary>). Se despliega el primer nodo antes de leer su
          // enlace y hacer clic, para confirmar que el flujo completo (abrir →
          // ver lecciones → "Empezar") sigue sin bloqueos.
          const primerNodo = page.locator('details.mn-nodo').first();
          await primerNodo.locator('summary').click();
          const primerEnlace = primerNodo.locator('a[href^="#/pda/"]');
          const primerHref = await primerEnlace.getAttribute('href');
  ...
  // aquí con la misma verificación de flujo completo (probarPDA). ---
  {
    ...
    // grupo. Se verifica una sola vez, con una sesión aparte.
    {
      ...
      try {
        ...
        // Paso 20: mismo cambio que en el camino de grado — abrir el primer
        // nodo (<details class="mn-nodo">) antes de clicar su enlace interno.
        const primerNodoEjercitate = page.locator('details.mn-nodo').first();
        await primerNodoEjercitate.locator('summary').click();
        await primerNodoEjercitate.locator('a[href^="#/pda/ejercitate/"]').click();
